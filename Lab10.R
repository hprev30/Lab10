...
setwd(dir = "C:/Users/hjp4906/Desktop/Lab10/chlor_a")

#list netCDF files in your directory
c <- list.files(path ="C:/Users/hjp4906/Desktop/Lab10/chlor_a", pattern=".nc", full.names=F, recursive = T)
c
file <- c[1]
...
chl_sum <- chl %>% group_by(lon,lat)
  
chl_sum <- summarise(chl_sum, mean_chl = mean(value, na.rm = T),
                     std_chl = sd(value, na.rm = T),
                     n_chl = n())
mean_chl <- mean(chl_sum$value, na.rm = T)
std_chl <- sd(chl_sum$value, na.rm =T)
var_chl <- var(chl_sum$value, na.rm =T)
sum_chl <- summarise(chl_sum, var_chl = var(value, na.rm = T))
...
# save csv file
write.table(chl_sum,"sum_stat", row.names=FALSE)
